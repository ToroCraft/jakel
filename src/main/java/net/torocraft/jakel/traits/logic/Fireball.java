...
public class Fireball {
  ...
  private static void smallFireball(EntityLivingBase entity, EntityLivingBase target) {
    ...
    double dy = target.getEntityBoundingBox().minY + (double) (target.height / 2.0F) - (entity.posY
        + (double) (entity.height / 2.0F));
    double dz = target.posZ - entity.posZ;

    float f = MathHelper.sqrt(MathHelper.sqrt(distanceSq)) * 0.5F;

    EntitySmallFireball entitysmallfireball = new EntitySmallFireball(entity.world, entity,
        dx + entity.getRNG().nextGaussian() * (double) f, dy,
    ...
    entity.world.playEvent(null, 1018,
  ...
  private static void bigFireball(EntityLivingBase entity, World world, EntityLivingBase target) {
    ...
    double d3 =
        target.getEntityBoundingBox().minY + (double) (target.height / 2.0F) - (0.5D + entity.posY
            + (double) (entity.height / 2.0F));
  ...
  public static void handleHeatTraitUpdate(EntityLiving entity) {
    World world = entity.world;
    int heatDistance = 8;

    List<EntityPlayer> playersToCook = world
        .getEntitiesWithinAABB(EntityPlayer.class,
            new AxisAlignedBB(entity.getPosition()).grow(heatDistance, heatDistance, heatDistance));
