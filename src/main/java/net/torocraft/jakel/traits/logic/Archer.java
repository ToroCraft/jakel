...
public class Archer {
  ...
  private static Potion[] TIPPED_ARROWS = {MobEffects.HUNGER, MobEffects.POISON,
      MobEffects.WEAKNESS, MobEffects.SLOWNESS};
  ...
  public static void onDrop(List<EntityItem> drops, EntityCreature nemesisEntity, Trait trait) {
    drops.add(TraitApi
        .drop(nemesisEntity, new ItemStack(Items.ARROW, nemesisEntity.getRNG().nextInt(64))));
    if (trait.level > 3) {
      ItemStack arrows = new ItemStack(Items.TIPPED_ARROW, nemesisEntity.getRNG().nextInt(64));
      PotionUtils.addPotionToItemStack(arrows, PotionType
          .getPotionTypeForName(DROP_TYPES[nemesisEntity.getRNG().nextInt(DROP_TYPES.length)]));
  ...
  public static void attackWithArrow(EntityLiving archer, EntityLivingBase target, int level) {
    ...
    archer.playSound(SoundEvents.ENTITY_SKELETON_SHOOT, 1.0F,
        1.0F / (Trait.rand.nextFloat() * 0.4F + 0.8F));
  ...
  private static void setPoisonArrow(Random rand, EntityArrow entityarrow, int level) {
    ...
    ((EntityTippedArrow) entityarrow).addEffect(
        new PotionEffect(TIPPED_ARROWS[rand.nextInt(TIPPED_ARROWS.length)], 400 * level));
  }
}
