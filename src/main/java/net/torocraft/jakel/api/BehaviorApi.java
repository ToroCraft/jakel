...
public class BehaviorApi {
  ...
  private static void startWorshiping(EntityLiving entity, int count) {
    entity.getTags().add(Jakel.TAG_WORSHIPING);
    entity.getEntityData().setTag(Jakel.NBT_WORSHIP_COOLDOWN, new NBTTagInt(count));
    cancelAllAITasks(entity);
  }

  public static void stopWorshiping(EntityLiving entity) {
    entity.getEntityData().removeTag(Jakel.NBT_WORSHIP_COOLDOWN);
    entity.getTags().remove(Jakel.TAG_WORSHIPING);
  ...
  public static boolean isWorshiping(EntityLiving entity) {
    return entity.getTags().contains(Jakel.TAG_WORSHIPING) && entity.getEntityData()
        .hasKey(Jakel.NBT_WORSHIP_COOLDOWN);
  }
